...
Notice that there are two files specified in the call to `gcc` to make the `a.out` executable. The first file [y86-64.c](y86-64.c) is where your code goes. The second file, [util.c](util.c) implements the machine state with all the needed functions to read and update it. The functions needed for the lab are all defined in [util.h](util.h) for convenience.
...
# Where to start?
 ...
 2. Study the provided machine interface in [util.h](util.h) that reads and updates the machine state with the various constants defined for convenience in implementing the lab
